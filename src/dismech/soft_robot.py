...
from .springs import BendTwistSpring, StretchSpring, StretchNodeSpring, HingeSpring, TriangleSpring, StretchBendTwistSpring
...
class SoftRobot:
    ...
    def _init_springs(self, geo: Geometry, geom: GeomParams):
        ...
        # Stretch node springs
        self.__stretch_node_springs = [StretchNodeSpring(spring,
                                                         self.__ref_len[i],
                                                         self.__EA,
                                                         self.map_node_to_dof)
                                        for i, spring in enumerate(geo.bend_twist_springs)]
        ...
        # Stretch-bend-twist spring
        self.__stretch_bend_twist_springs = [
            StretchBendTwistSpring(spring,
                                   sign,
                                   self.__ref_len[i],
                                   self.__ref_len,
                                   self.__EA,
                                   ...
                                   self.map_node_to_dof,
                                   self.map_edge_to_dof)
            for i, (spring, sign) in enumerate(zip(geo.bend_twist_springs, geo.bend_twist_signs))
        ]
    ...
    def _get_rod_width_and_height(self) -> typing.Tuple[float, float]:
        """Get current rod cross-section width w and height h from geometry."""
        h = float(self.__geom.rod_r0) if self.__geom.rod_r0 is not None else 0.0
        if h <= 0:
            return 0.0, 0.0
        if self.__geom.axs is not None and self.__geom.axs > 0:
            w = float(self.__geom.axs) / h
        else:
            w = 0.0
        return w, h

    def update_rod_geometry(
        self,
        width: float | None = None,
        height: float | None = None,
    ) -> "SoftRobot":
        """
        Homotopically update rod geometry (width and/or height). Length cannot be changed
        as it is tied to node positions and DOFs.
        Mutates geom, stiffness, springs, and mass matrix in place. Returns self.
        """
        w, h = self._get_rod_width_and_height()
        if width is not None:
            w = float(width)
        if height is not None:
            h = float(height)
        if w <= 0 or h <= 0:
            raise ValueError(f"Rod width and height must be positive, got w={w}, h={h}")

        # Update GeomParams (rod cross-section: w x h)
        self.__geom.rod_r0 = h
        self.__geom.axs = w * h
        self.__geom.ixs1 = w * h**3 / 12
        self.__geom.ixs2 = h * w**3 / 12
        self.__geom.jxs = w * h**3 / 3

        # Recompute rod stiffness
        self.__EA, self.__EI1, self.__EI2, self.__GJ = compute_rod_stiffness(
            self.__geom, self.__material
        )

        # Update StretchBendTwistSprings
        for s in self.__stretch_bend_twist_springs:
            s.EA = self.__EA
            s.EI1 = self.__EI1
            s.EI2 = self.__EI2
            s.GJ = self.__GJ
        ...
        # Update BendTwistSprings
        for s in self.__bend_twist_springs:
            s.stiff_EI = np.array([self.__EI1, self.__EI2])
            s.stiff_GJ = self.__GJ

        # Update rod StretchSprings (first n_rod edges)
        n_rod = self.__n_edges_rod_only
        for s in self.__stretch_springs[:n_rod]:
            s.EA = self.__EA

        # Update StretchNodeSprings
        for s in self.__stretch_node_springs:
            s.EA = self.__EA

        # Recompute mass matrix
        self.__mass_matrix[:] = self._get_mass_matrix(self.__geom, self.__material)

        return self

    def update_rod_material(
        self,
        density: float | None = None,
        youngs_rod: float | None = None,
        poisson_rod: float | None = None,
    ) -> "SoftRobot":
        """
        Homotopically update rod material parameters. Mutates material, stiffness,
        springs, and mass matrix in place. Returns self.
        """
        if density is not None:
            self.__material.density = float(density)
        if youngs_rod is not None:
            self.__material.youngs_rod = float(youngs_rod)
        if poisson_rod is not None:
            self.__material.poisson_rod = float(poisson_rod)
    ...
    @property
    def stretch_node_springs(self) -> typing.List[StretchNodeSpring]:
        """List of stretch node spring elements"""
        return self.__stretch_node_springs
    
    @property
    def stretch_bend_twist_springs(self) -> typing.List[StretchBendTwistSpring]:
        """List of stretch-bend-twist spring elements"""
        return self.__stretch_bend_twist_springs
    ...
    @property
    def rod_r0(self) -> np.ndarray:
        """Rod radius (n_edges,)"""
        return self.__geom.rod_r0
    
    # Stiffness properties

    @property
    def EA(self) -> np.ndarray:
        """Axial stiffness (n_edges,)"""
        return self.__EA
    
    @property
    def EI1(self) -> np.ndarray:
        """Bending stiffness (n_edges,)"""
        return self.__EI1
    
    @property
    def EI2(self) -> np.ndarray:
        """Bending stiffness (n_edges,)"""
        return self.__EI2
    
    @property
    def GJ(self) -> np.ndarray:
        """Twisting stiffness (n_edges,)"""
        return self.__GJ
