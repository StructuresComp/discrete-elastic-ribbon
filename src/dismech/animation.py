...
class AnimationOptions:
    ...
    skip_frames: int = 1  # Add this line


def get_animation(robot, t, qs, options: AnimationOptions):
    # Apply frame skipping
    t = t[::options.skip_frames]
    qs = qs[::options.skip_frames]
    
    # Ensure t and qs have the same length after slicing
    min_len = min(len(t), qs.shape[0])
    t = t[:min_len]
    qs = qs[:min_len]
...
def get_interactive_animation_plotly(robot, t, qs, options: AnimationOptions):
    ...
    r = np.linalg.norm(np.ptp(nodes_all, axis=(0, 1))) * options.zoom_factor  # Radius for camera distance (adjust as needed)
    camera_eye = dict(
