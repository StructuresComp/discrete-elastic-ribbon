...
class AnalyticalSadowskyElasticEnergy(nn.Module):
    """
    Analytical Sadowsky-model elastic-energy for a 3D rod.
    ...
    Computes dimensionless energy density E_nn such that
      E_full = ½ EA Δl (Δε)² + ½ (EI1/Δl)(Δκ1)² + ½ (EI2/Δl)(Δκ2)²
             + ½ (GJ/Δl)(Δτ)² + ½ (EI1/Δl)·(Δτ)⁴/(Δκ1)²
    and E_nn = E_full / (½ EA Δl).

    Note: The nonlinear term uses regularization τ⁴/(κ₁² + ε²) to prevent numerical 
    instability when κ₁ is small. This smoothly approaches 0 as κ₁ → 0.
    ...
    def __init__(self,
        ...
        self.h = float(h)
        self.eps = 1e-6  # Regularization parameter for k1^2 in denominator to prevent explosion
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Sadowsky nonlinear term: (EI1/Δl) * (τ⁴/κ₁²)
        # Use regularization: τ⁴/(κ₁² + ε²) to prevent explosion when k1 is small
        # This smoothly approaches 0 as k1 → 0, and approaches τ⁴/κ₁² when |k1| >> ε
        k1_squared = k1**2
        k1_squared_reg = k1_squared + self.eps**2
        E_nonlinear = half * (self.EI1 * self.inv_dl) * (tau**4 / k1_squared_reg)
