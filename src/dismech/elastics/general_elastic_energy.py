...
class GeneralElasticEnergyRod3D(ElasticEnergy):
    """
    ...
    def __init__(self,
                ...
                #  nn_model: ElasticEnergyRod3DNN,
                 nn_model: AnalyticalSanosElasticEnergy,
    ...
    def get_energy_linear_elastic(self, state: RobotState, output_scalar: bool = True):
        ...
        # Iterate over springs (batch processing in NN might be faster if implemented)
        for i in range(n_springs):
            strain_input = strains_input_nn[i, :]
            # Pass scaled delta strains to NN model
            nn_energy_output, _, _ = self.nn_model.compute_energy_grad_hess(strain_input)

            # Scale NN output by physical parameters (0.5 * EA * l_eff)
            energy_i = 0.5 * self.EA[i] * self.l_eff[i] * nn_energy_output
            energies[i] = energy_i
    ...
    def grad_hess_energy_linear_elastic(self, state: RobotState, sparse: bool = False) -> typing.Tuple[np.ndarray, np.ndarray] | typing.Tuple[np.ndarray, sp.csr_array]:
        ...
        # 4. Iterate through springs
        for i in range(n_springs):
            # Get NN derivatives w.r.t scaled delta strains
            _, gradE_strain_nn, hessE_strain_nn = self.nn_model.compute_energy_grad_hess(strains_input_nn[i, :])
            # Shapes: (4,), (4, 4)

            # Get scaled strain derivatives for this spring
            G_strain_dof_s = grad_strains_dof_scaled[i, :, :] # Shape (11, 4)
            H_strain_dof_s = hess_strains_dof_scaled[i, :, :, :] # Shape (11, 11, 4)

            # # Apply chain rule for gradient
            gradE_dof_local = np.einsum('k,jk->j', gradE_strain_nn, G_strain_dof_s) # Shape (11,)

            # Apply chain rule for Hessian
            term1 = np.einsum('k,ijk->ij', gradE_strain_nn, H_strain_dof_s) # Shape (11, 11)
            # Correct calculation for the second term of the Hessian chain rule: sum_{k,l} H_kl * G_ik * G_jl
            term2_outer = np.einsum('ik,jl->ijkl', G_strain_dof_s, G_strain_dof_s) # Shape (11, 11, 4, 4)
            term2 = np.einsum('kl,ijkl->ij', hessE_strain_nn, term2_outer) # Shape (11, 11)
            hessE_dof_local = term1 + term2 # Shape (11, 11)
            ...
            # diag_terms = 0
            # for k in range(4):  # Loop over strain components
            #     G_k = G_strain_dof_s[:, k]  # Gradient of kth strain component
            #     diag_terms += hessE_strain_nn[k, k] * np.outer(G_k, G_k)

            # # Cross-derivative terms
            # cross_terms = 0
            # for k in range(4):
            #     for l in range(k+1, 4):  # Only upper triangle to avoid double counting
            #         G_k = G_strain_dof_s[:, k]
            #         G_l = G_strain_dof_s[:, l]
            #         cross_terms += hessE_strain_nn[k, l] * (np.outer(G_k, G_l) + np.outer(G_l, G_k))

            # hessE_dof_local = term1 + diag_terms + cross_terms # Shape (11, 11)

            # Scale by physical factor (0.5 * EA * l_eff)
            scale_factor = 0.5 * self.EA[i] * self.l_eff[i]
            gradE_dof_local *= scale_factor
            hessE_dof_local *= scale_factor

            # Assemble into global gradient/Hessian
            dof_indices = self._ind[i, :] # Global DOFs for this spring
            Fs[dof_indices] -= gradE_dof_local # Subtract gradient for force
            ...
            Js_data.append(-hessE_dof_local.ravel())
