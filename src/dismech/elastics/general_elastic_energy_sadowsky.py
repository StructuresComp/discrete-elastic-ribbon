...
class GeneralElasticEnergySadowsky(ElasticEnergy):
    """
    General elastic energy for 3D rods using the Sadowsky analytical energy model.
    ...
    def __init__(self,
                 ...
                 nn_model: AnalyticalSadowskyElasticEnergy,
    ...
    def _get_adjusted_material_directors(self, state: RobotState):
        ...
        if self._edges_ind is not None and self._sgn is not None:
             ...
             raise NotImplementedError("GeneralElasticEnergySadowsky requires springs with 'edges_ind' and 'sgn' attributes, or RobotState needs modification.")
    ...
    def get_energy_linear_elastic(self, state: RobotState, output_scalar: bool = True):
        """
        Computes the total elastic energy using the Sadowsky analytical model with batch processing.
    ...
    def grad_hess_energy_linear_elastic(self, state: RobotState, sparse: bool = False) -> typing.Tuple[np.ndarray, np.ndarray] | typing.Tuple[np.ndarray, sp.csr_array]:
        """
        Computes the gradient and Hessian of the total elastic energy using the Sadowsky analytical model.
