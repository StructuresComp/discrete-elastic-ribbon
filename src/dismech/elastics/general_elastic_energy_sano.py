...
class GeneralElasticEnergySano(ElasticEnergy):
    """
    General elastic energy for 3D rods using the Sano analytical energy model.
    ...
    def refresh_params_from_springs(self, springs: typing.List[StretchBendTwistSpring],
                                    sano_zeta: float | None = None, **kwargs):
        """
        Refresh EA, EI1, EI2, GJ, h, l_eff from springs for homotopy.
        Also updates the analytical nn_model. Pass sano_zeta when geometry (w, h) changed:
        sano_zeta = sqrt(w^4 / (120 * h^2)). If None, keeps current nn_model.zeta.
        """
        self.EA = np.array([s.EA for s in springs], dtype=np.float64)
        self.EI1 = np.array([s.EI1 for s in springs], dtype=np.float64)
        self.EI2 = np.array([s.EI2 for s in springs], dtype=np.float64)
        self.GJ = np.array([s.GJ for s in springs], dtype=np.float64)
        self.h = np.array([s.h for s in springs], dtype=np.float64)
        self.l_eff = np.array([s.ref_len for s in springs], dtype=np.float64)
        if springs:
            s0 = springs[0]
            zeta = sano_zeta if sano_zeta is not None else getattr(
                self.nn_model, 'zeta', np.sqrt(float(s0.ref_len)**2 / (120.0 * float(s0.h)**2))
            )
            self.nn_model.update_params(
                EA=float(s0.EA), EI1=float(s0.EI1), EI2=float(s0.EI2), GJ=float(s0.GJ),
                delta_l=float(s0.ref_len), zeta=zeta, h=float(s0.h)
            )
        self._stiffness_k = self.EA * self.l_eff
    ...
    def _get_adjusted_material_directors(self, state: RobotState):
        ...
        if self._edges_ind is not None and self._sgn is not None:
             ...
             raise NotImplementedError("GeneralElasticEnergySano requires springs with 'edges_ind' and 'sgn' attributes, or RobotState needs modification.")
    ...
    def get_energy_linear_elastic(self, state: RobotState, output_scalar: bool = True):
        """
        Computes the total elastic energy using the Sano analytical model with batch processing.
    ...
    def grad_hess_energy_linear_elastic(self, state: RobotState, sparse: bool = False) -> typing.Tuple[np.ndarray, np.ndarray] | typing.Tuple[np.ndarray, sp.csr_array]:
        """
        Computes the gradient and Hessian of the total elastic energy using the Sano analytical model.
        ...
        # 4. Batch computation: compute energy, gradient, and Hessian for all elements
        ...
        # Shapes: (n_springs, 4), (n_springs, 4, 4)
