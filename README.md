...
Each analytical module provides the energy density \(E(\tilde{\boldsymbol{\epsilon}})\) as a function of normalized strains \(\tilde{\boldsymbol{\epsilon}} = [\Delta\varepsilon, \Delta\kappa^{(1)}h/\ell, \Delta\kappa^{(2)}h/\ell, \Delta\tau h/\ell]\), plus gradient and Hessian.
...
1. **Implement the analytical energy** in `src/dismech/elastics/`:
   - Create `analytical_<name>_elastic_energy.py` with a class that provides:
     - `forward(x)` or equivalent: energy density \(E(\tilde{\boldsymbol{\epsilon}})\) for input `x` of shape `(batch, 4)`.
     - `compute_energy_grad_hess(x)`: returns `(E, ∇E, ∇²E)` w.r.t. normalized strains.
...
2. **Implement the general elastic energy wrapper**:
   - Create `general_elastic_energy_<name>.py` that:
     ...
     - Calls your analytical model with the normalized strain vector \(\tilde{\boldsymbol{\epsilon}} = [\Delta\varepsilon, \Delta\kappa^{(1)}h/\ell, \Delta\kappa^{(2)}h/\ell, \Delta\tau h/\ell]\).
     - Assembles forces and stiffness via the chain rule as in Algorithm 3.

3. **Register in the time stepper** (`time_stepper.py`):
   - Add the new model to the `energy_model_type` dispatch and construct the analytical model with geometry-derived parameters (EA, EI1, EI2, GJ, delta_l, h, and any model-specific parameters like Sano’s ζ or Wunderlich’s W).

Only the analytical energy expression \(E(\tilde{\boldsymbol{\epsilon}})\) needs to be implemented; the rest is handled by the generalized framework.
...
# Plot Hmid vs ΔW (with FEA reference and shear force)
